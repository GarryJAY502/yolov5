...
FILE = Path(
    __file__).resolve()  # Path(__file__)获取当前脚本文件路径，resolve()将路径解析为绝对路径，解析出所有符号链接、相对路径段和上级目录('..')等，此处表示detect.py位置
ROOT = FILE.parents[0]  # YOLOv5 root directory
if str(ROOT) not in sys.path:  # 如果根路径不在系统path（模块的查询路径列表）中，则添加入系统path，确保Python能够导入YOLOv5项目中的模块和包。
    sys.path.append(str(ROOT))  # add ROOT to PATH
# os.path.relpath()函数计算从当前工作目录到ROOT的相对路径。
# Path.cwd()返回当前工作目录。++
# 这一步的目的是将ROOT变量更新为相对于当前工作目录的路径
...
def run(
    ...
    """
    # 处理预测路径
    source = str(source)  # 输入的命令行，转换为字符串
    save_img = not nosave and not source.endswith(".txt")  # save inference images 是否保存推理后的图片
    is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS)  # 判断是否在这些图片或视频格式中
    is_url = source.lower().startswith(("rtsp://", "rtmp://", "http://", "https://"))  # 判断是否为网络流地址
    # 判断是否是应该数值，如摄像头路径，默认摄像头为0，或判断是否为摄像头流媒体文件，或判断是否不是图片并且是有效的url链接
    webcam = source.isnumeric() or source.endswith(".streams") or (is_url and not is_file)
    screenshot = source.lower().startswith("screen")  # 用于判断变量 source 是否表示一个屏幕截图的指令。
    # 如果是一个有效的url或文件，则检查下载文件
    ...
    # increment_path增量路径，会检测文件夹下　ath(project) / name　的数字到多少，递增命名
    # 如果 exist_ok 为 True，即使路径已经存在，函数也不会尝试递增路径，而是直接返回原始路径。
    # 如果 exist_ok 为 False，当路径已经存在时，函数会尝试递增路径，直到找到一个不存在的路径为止。
    save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment run
    # 添加labels文件夹，存储标签结果到txt文件
...
def parse_opt():
    ...
    # 对image size做设置，默认640，只有应该值的时候，则字典扩充成[640,640]（列表乘法）
